/// <summary> InvertedGenerationalDistance.java
/// 
/// </summary>
...
namespace JARE.qualityIndicator
{
	...
	/// <summary> This class implements the inverted generational distance metric. 
	...
	public class InvertedGenerationalDistance
	{
		...
		/// <summary> Returns the inverted generational distance value for a given front</summary>
		/// <param name="front">The front 
		/// </param>
		/// <param name="trueParetoFront">The true pareto front
		/// </param>
		public virtual double invertedGenerationalDistance(double[][] front, double[][] trueParetoFront, int numberOfObjectives)
		{
			...
			*/
			...
			for (int i = 0; i < normalizedParetoFront.Length; i++)
				sum += System.Math.Pow(m_utils.distanceToClosedPoint(normalizedParetoFront[i], normalizedFront), m_pow);
		...
		public static void  Main(System.String[] args)
		{
			if (args.Length < 2)
			{
				System.Console.Error.WriteLine("InvertedGenerationalDistance::Main: Usage: java " + "InvertedGenerationalDistance <FrontFile> " + "<TrueFrontFile>  <numberOfObjectives>");
			...
			double val = qualityIndicator.invertedGenerationalDistance(solutionFront, trueFront, (System.Int32.Parse(args[2])));
